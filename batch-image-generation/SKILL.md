...
Skill 一旦被激活，先且只根据**当前用户消息**校验输入路径和完整 Prompt；在确认两项均有效前，不得列目录、读取输入图片、读取脚本/依赖、寻找测试素材或调用任何命令。

- 单独提供输入路径只满足输入项，仍缺完整 Prompt，必须立即输出申领单；不得据此补写 Prompt、猜测参考图或开始 `--dry-run`。
...
2. 用三个反引号的代码块包裹后原样输出 `prompts/参数申领单.txt` 全文，保留原始换行和空格，不得压缩成单段连续文本；一次性给出全部待填项。不得逐项追问、不得删减选项、不得改写措辞或调整分区顺序。
...
- 硬停点：先且只根据当前用户消息校验；必填项（输入路径、Prompt）缺失时先输出参数申领单并停止，禁止调用脚本、寻找自测素材、沿用历史 task 或使用备份副本；参数齐全后，真实调用前必须执行 `--dry-run`，人工核对任务类型、条目数量、输出目录与 Prompt 预览后才可继续。
