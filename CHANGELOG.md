...
## 2026-08-19 · image-element-crop 契约 v2 → v3

- 变了什么：新增「激活后的第一动作」——只按当前用户消息先校验必填项；缺参时先输出申领单，禁止以测试、示例、历史 task 或备份副本为由探查目录、寻找素材或调用脚本。
- 为什么变：修复缺参调用被错误当作自测任务，进而执行历史备份中的旧版本脚本的问题。
- 受影响工作流：（暂无，当前无工作流依赖此 skill）
- 使用者要做什么：无需调整调用方式；只提交输入路径时会先收到完整申领单，需补齐目标尺寸和输出目录后才会执行。

## 2026-08-19 · batch-image-generation 契约 v5 → v6

- 变了什么：新增「激活后的第一动作」——只按当前用户消息先校验输入路径和完整 Prompt；缺参时先输出申领单，禁止以测试、示例、历史 task 或备份副本为由探查目录、寻找素材或调用脚本。
- 为什么变：统一防止缺参调用被误当作自测任务，从历史上下文或旧副本猜测参数后继续执行。
- 受影响工作流：`workflow-batch-generate-normalize`
- 使用者要做什么：无需调整调用方式；只提交输入路径时会先收到完整申领单，需补齐完整 Prompt 后才会执行。
